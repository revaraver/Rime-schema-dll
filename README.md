# 单独维护的编译好的长期使用的rime配置 几乎已经满足各种日常中英混输的使用
 - 主要改动:
    - 1.使用左右方向键进行翻页,对代码中的.输入进行了完美规避, 但是需要搭配caps脚本进行caps+jl的比较方便的翻页. 然后翻页其实日常使用时用到的频率是比较小的所以其实是完全可以接收由单键改为双键的
    - 2.去掉了原本雾凇中大量的日常使用不到的加载器,然后现在几乎感受不到打字时的卡顿
    - 3.自编译的rime.dll中添加了editor中的commit_raw_input_and_send_space功能 自编译,发送原始字符后加一个空格. 然后把这个功能绑定到了日常不会去按的Shift_R键. 这个是核心,从此可以在中文模式下比较愉快的进行英文的输入 只是需要一段时间的使用shift_R来代替空格的功能. 在输入比如var 变量: 之类的语句时,var之后不需要再先回车再空格这种比较难受的输入,但其实还是远远不如双空格键盘方案. 只是双空格方案又会有自己的适应期
    - 4.句号会自己上屏前面的输入并添加一个句号, 左括号亦如是, 能够比较快速的进行变量方法的调用输入
    - 5.删去预输入中的拼音分词之间的空格与' 符号,这是最关键的然后是中英文混输的最重要前提与基础
  - 更好的方案?
