# 单独维护的编译好的长期使用的rime配置 几乎已经满足各种日常中英混输的使用
 - 主要改动:

    - 使用左右方向键进行翻页,对代码中的.输入进行了完美规避, 但是需要搭配caps脚本进行caps+jl的比较方便的翻页. 然后翻页其实日常使用时用到的频率是比较小的所以其实是完全可以接收由单键改为双键的

    - 去掉了原本雾凇中大量的日常使用不到的加载器,然后现在几乎感受不到打字时的卡顿

    - 自编译的rime.dll中添加了editor中的commit_raw_input_and_send_space功能 自编译,发送原始字符后加一个空格. 然后把这个功能绑定到了日常不会去按的Shift_R键. 这个是核心,从此可以在中文模式下比较愉快的进行英文的输入 只是需要一段时间的使用shift_R来代替空格的功能. 在输入比如var 变量: 之类的语句时,var之后不需要再先回车再空格这种比较难受的输入,但其实还是远远不如双空格键盘方案. 只是双空格方案又会有自己的适应期

    - 句号会自己上屏前面的输入并添加一个句号, 左括号亦如是, 能够比较快速的进行变量方法的调用输入

    - 删去预输入中的拼音分词之间的空格与' 符号,这是最关键的然后是中英文混输的最重要前提与基础

  - 更好的方案?

    - 需要去重构weasel的rime的方案,旨在让weasel去旁通任何输入,同时自维护一个输入框,这样就能做到任何的输入都能对ide中的补全进行触发. 但是按12345等选择键的时候又能从自维护的输入框从选择对应的中文对之前的输入进行替换.但他具有一系列难点与状态树的不完善.难点体现在weasel编译的难度(编译了一天都失败系列),重构整个项目的难度,与windows深层dll进行交互的难度,完美取代之前输入字符的难度(虽然可以使用Ctrl+shift+left这种方案对前面而选中但是不是完美的方案) 状态树的难点体现在翻页键,如果使用左右方向键则会打乱正常的左右,使用,.亦会打乱正常的,.的输入

  - 总之是目前而言能维护的一种极限了,长时间都会使用下去,特定版本的rime在另一个仓库中有备份,dll与配置在这个仓库中有备份.然后日常使用是建议配合inputtip对鼠标进行变换来进行更好的中英文指示,不过当这个输入法唯一中英混输状态使用固定后相比再也无须inputtip来对当前是中文还是英文的输入状态进行任何提示.

  2025_11_01 20:11:13更新

  新增了三个额外的功能,现在自编译的总共有四个

    # commit_raw_input_and_send_space #自编译,发送原始字符后加一个空格
  
    # commit_raw_input_and_send_enter #发送原始字符后加一个回车
  
    # confirm_and_send_space #上屏候选后发送一个空格
  
    # confirm_and_send_enter #上屏候选后发送一个回车

  废除了shift_l的切换英文功能,从此以后再也不需要更改输入法状态

  现在shift_l是直接上屏英文,shift_r是上屏英文后空格 

  ctrl也是上屏英文,目前的扩展只是给shift_r进行了英文+空格,看看后续使用是否不便需要给他添加
